...
def preconstruct_entity_sec_task(gt, e_id_sec_embed):
    ...
    return X_E, X_F, sec_sec_E, sec_sec_F
...
def construct_entity_sec_task(gt, e_id_sec_embed, k = 100):

    print("construct_entity_sec_task")

    data = load_file(config.get_path("task") / "entity_sec_task"  / "data", ftype="pkl")
    label = load_file(config.get_path("task") / "entity_sec_task"  / "label", ftype="pkl")
    ...
    if data != None:
        ...
        X_E, X_F, sec_sec_E, sec_sec_F = preconstruct_entity_sec_task(gt, e_id_sec_embed)
        ...
        store_file(config.get_path("task") / "entity_sec_task"  / "data", (X_1, X_2, Y), "pkl")
        store_file(config.get_path("task") / "entity_sec_task"  / "label", L, "pkl")
...
if __name__ == "__main__":
    ...
    data, labels = construct_entity_sec_task(ally_enemy_pairs, e_id_sec_embed, k=5000)
