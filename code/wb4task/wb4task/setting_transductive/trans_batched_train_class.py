...
class Trainer(ABC):
    ...
    def train_model(self, dl, model, graph, n_epochs=32, early_stop=3, lr = 0.0001, weight_decay=1e-5):
        ...
        for epoch in range(n_epochs):
            ...
            for i, (input_nodes, edge_subgraph, blocks) in enumerate(dl):
                ...
                y_hat, y, y_val_hat, y_val = self.pass_data_to_model(model, "train", edge_subgraph, blocks)
    ...
    def evaluate_model(self, model, dl):
        with torch.no_grad():  ## turn off gradients for validation
            ...
            for i, (input_nodes, edge_subgraph, blocks) in enumerate(dl):
                ...
                #self.setup_task(edge_subgraph, blocks, self.wikidataset.g)
                y_hat, y, _, _ = self.pass_data_to_model(model, "test", edge_subgraph, blocks)
