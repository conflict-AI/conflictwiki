...
class Trainer(ABC):
    ...
    def __init__(self, wikidataset, entity_emb = "c"):
    ...
    def train_model(self, train_dl, val_dl, model, n_epochs=32, early_stop=3, lr = 0.0001, weight_decay=1e-5):
        ...
        for epoch in range(n_epochs):
            ...
            train_loss = 0
            train_acc = 0
            for i, (x_1, x_2, c, e, y, l) in enumerate(train_dl):

                x_1, x_2 = self.entity_emb_selection(x_1, x_2, self.entity_emb)
                optimizer.zero_grad()

                ## function to implement
                y_hat = self.pass_data_to_model(model, x_1, x_2, c, e, y, l)
            ...
            with torch.no_grad():  ## turn off gradients for validation
                for i, (x_1, x_2, c, e, y, l) in enumerate(val_dl):
    ...
    def evaluate_model(self, model, test_dl):
        with torch.no_grad():  ## turn off gradients for validation
            ...
            E = self.wikidataset.E[test_dl.dataset.indices]
            C = self.wikidataset.C[test_dl.dataset.indices]
            ...
            Y = Y.detach().numpy()
            prec, rec, f1, support = precision_recall_fscore_support(Y, y_hat_hot, average="weighted")  # macro
            print("\nroc_auc:", roc_auc, "prec:", prec, "rec:", rec, "f1:", f1, "support:", support)


    def entity_emb_selection(self, x_1_batch, x_2_batch, ent_type = "c"):
        if ent_type == "c":
            return x_1_batch[:, 0, :], x_2_batch[:, 0, :]
        elif ent_type == "e":
            return x_1_batch[:, 1, :], x_2_batch[:, 1, :]
        elif ent_type == "e_sec": ## only one dimension anyways
            return x_1_batch[:, :], x_2_batch[:, :]
        elif ent_type == "e_c": ## return both
            return (x_1_batch[:, 0, :], x_1_batch[:, 1, :]), (x_2_batch[:, 0, :], x_2_batch[:, 1, :])


    @abstractmethod
    def pass_data_to_model(self,x_1, x_2):
