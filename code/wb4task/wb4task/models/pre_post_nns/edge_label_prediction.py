...
class EdgeLabelPredictor(nn.Module):
    ...
    def __init__(self, h_node_features, include_edge_features = False):
        super().__init__()
        #self.W = nn.Linear(2 * in_features, num_classes)
        #self.W = nn.Linear(2 * in_features, 1) ## exchange this with siamese architecture

        self.node_fc1 = nn.Linear(h_node_features, 12)
    ...
    def edge_node_feature_merge(self, node_features, edge_features):
    ...
    def edge_wise_score(self, edges):
        ...
        if self.include_edge_features:
            src_node = self.edge_node_feature_merge(src_node, edge_features)
            dst_node = self.edge_node_feature_merge(dst_node, edge_features)
    ...
    def forward(self, edge_subgraph, n, e):
        ...
        with edge_subgraph.local_scope():
            ...
            edge_subgraph.edata['edge_h_features'] = e  ## assign hidden features from message passing to nodes
