...
class EdgeLabelPredictor(nn.Module):
    ...
    def __init__(self, h_node_features, dot_product_dim = 4, include_edge_features = False):
        ...
        self.node_fc1 = nn.Linear(3*h_node_features, 1)
    ...
    def concat_edge_node_feature(self, node0_features, node1_features, edge_features):
        # node_edge_mean = node_features - edge_features ## pairwise distance
        node_edge_mean = torch.cat((node0_features, node1_features, edge_features), 1)
        return node_edge_mean
    ...
    def edge_wise_score(self, edges):
        ...
        h = self.concat_edge_node_feature(src_node, dst_node, edge_features)
        merge_out = self.encoder(h)
